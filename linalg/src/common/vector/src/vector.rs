...
use super::chunks_pre_post::{
    ChunksWithPrePostSlice, ChunksWithPrePostSliceMut,
};
...
impl<T> Vector<T> {
    ...
    ///
    /// ## Performance
    /// If multiple small stream of elements are being inserted, then
    /// use method `push` instead
    #[inline]
    ...
    /// Returns the mutable slice group that shrinks
    #[inline(always)]
    pub fn chunks_with_pre_and_post(
        &self,
        size: usize,
    ) -> ChunksWithPrePostSlice<'_, T> {
        unsafe {
            ChunksWithPrePostSlice::new(
                from_raw_parts_mut(self.ptr, self.len),
                size,
            )
        }
    ...
    pub fn chunks_with_pre_and_post_mut(
        &self,
        size: usize,
    ) -> ChunksWithPrePostSliceMut<'_, T> {
        unsafe {
            ChunksWithPrePostSliceMut::new(
                from_raw_parts_mut(self.ptr, self.len),
                size,
            )
        }
        ...
        usize: From<T>,
    {
        unsafe {
            ...
            for value in range {
                *ptr.add(usize::from(value - start)) = value;
            }
            Self {
                ptr,
                layout,
