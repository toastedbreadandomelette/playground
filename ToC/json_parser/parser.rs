...
pub mod byte_parsing {
    ...
    macro_rules! read_byte {
        ($parser:ident) => ({
            ...
            if chr == Option::None { return Result::Error(Error::ParsingError(ParseError::EndOfBufferError)); }
            chr.unwrap()
        })
    }
    ...
    macro_rules! assert_curr_byte {
        ($parser:ident, $byte_val:expr) => { 
            if $parser.curr_byte != $byte_val {
                return Result::Error(Error::ParsingError(ParseError::UnexpectedTokenError($parser.curr_byte as char)))
            }
        }
    }

    macro_rules! get_closing_container {
        ($chr:expr) => { if $chr == b'[' { b']' } else if $chr == b'{' { b'}' } else if $chr == b'(' { b')' } else { b'\0' } }
    }
    ...
    impl Parser {
        ...
        pub fn parse_str(&mut self) -> Result<Container> {
            ...
            match result_container {
                ...
                Result::Error(val) => { println!("{}", val); Result::Error(val) },
                Result::None       => { println!("Technical issue"); Result::None },
            }
        }
        ...
        fn read_string_in_quotes(&mut self, end_quote_expected: u8) -> Result<Container> {
            ...
            loop {
                match read_byte!(self) {
                    ...
                    b'\\'        => { read_byte!(self); }
                    b'\'' | b'"' => { if self.curr_byte == end_quote_expected { break } else { } } 
                    _            => { }
                }
            }
            unsafe {
                Result::Ok(Container::Str(
        ...
        fn read_array_or_set(&mut self, end_bracket_expected: u8) -> Result<Container> {
            // Current byte is a quote, read and move to next one
            let mut array_container: Container = if end_bracket_expected == b']' { Container::new_array() } else { Container::new_set() };
            'parsing_array: loop {
                read_byte!(self);
                skip_whitespaces!(self);
                let curr_container = match self.curr_byte {
                    ...
                    b'['  | b'(' => { self.read_array_or_set(get_closing_container!(self.curr_byte)) }
                    b'{'         => { self.read_objects() }
                    b't'         => { expect_next_bytes!(self, b'r', b'u', b'e'); Result::Ok(Container::Boolean(true)) }
                    b'f'         => { expect_next_bytes!(self, b'a', b'l', b's', b'e'); Result::Ok(Container::Boolean(false)) }
                    b'n'         => { expect_next_bytes!(self, b'u', b'l', b'l'); Result::Ok(Container::Null) }
                    b']'  | b')'
                          | b'}' => { // End of current array/set
                        if self.curr_byte == end_bracket_expected {
                            break
                        } else {
                            Result::Error(Error::ParsingError(ParseError::ContainerParanthesisMismatchError{
                    ...
                    b'0'..=b'9' |
                    b'.' | b'-' |
                    b'+'         => { self.read_number(self.curr_byte) }
                    _            => { Result::Error(Error::ParsingError(ParseError::UnexpectedTokenError(self.curr_byte as char))) }
                };
                match curr_container {
                    Result::Ok(array_element) => { array_container.push(array_element); }
                    Result::Error(error_msg)  => { return Result::Error(error_msg); }
                    Result::None              => { break }
                }
                ...
                match self.curr_byte {
                    ...
                    b']' | b')' => {
                        if self.curr_byte == end_bracket_expected {
                            ...
                            return Result::Error(Error::ParsingError(ParseError::ContainerParanthesisMismatchError{
                                opening_container: end_bracket_expected as char,
                                closing_container: self.curr_byte as char
                            }));
                        }
                    }   // End of current array/set
                    _           => { println!("Unexpected {} {}", self.curr_column, self.curr_line); return Result::Error(Error::ParsingError(ParseError::UnexpectedTokenError(self.curr_byte as char))); }
                }
            }
        ...
        fn read_objects(&mut self) -> Result<Container> {
            ...
            'parsing_objects: loop {
                ...
                let verification = match key {
                    Result::Ok(val)         => { val },
                    Result::Error(err_msg)  => { return Result::Error(err_msg); }
                    Result::None            => { break }
                };
                ...
                let assoc_value = match self.curr_byte {
                    ...
                    b']' | b')'
                         | b'}'   => {
                        if self.curr_byte == b'}' {
                            return Result::Error(Error::ParsingError(ParseError::InvalidKeyValueFormatError{reading_key: verification.as_string().unwrap()}));
                        } else {
                            Result::Error(Error::ParsingError(ParseError::ContainerParanthesisMismatchError{
                                opening_container: '{',
                                closing_container: self.curr_byte as char
                ...
                match assoc_value {
                    Result::Ok(val)     => { object_container.insert(verification.as_string().unwrap(), val); }
                ...
                match self.curr_byte {
                    ...
                    _           => { return Result::Error(Error::ParsingError(ParseError::UnexpectedTokenError(self.curr_byte as char))); }
                }
            }
            // println!("Ret");
        ...
        fn read_number(&mut self, byte_read: u8) -> Result<Container> {
            ...
            'parsing_number: loop {
                match read_byte!(self) {
                    ...
                    _ => { return Result::Error(Error::ParsingError(ParseError::InvalidNumberParseError(self.curr_byte as char))); }
                }
            }
